    ...
    const layouts = mainRoutes.map(({ layout: Layout, routes }, index) => {
      ...
      return (
        <Route key={index} element={<Layout />}>
          {subRoutes.map(({ component: Component, path, name, isPublic, allowedUserType }) => {
            if (isPublic) {
              // Rota pública
              ...
              return (
                Component &&
                path && (
                  <Route key={name} element={<ProtectedRoute isAuthorized={isAuthorized} acesso={allowedUserType} />}>
