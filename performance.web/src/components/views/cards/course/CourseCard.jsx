...
export default function CourseCard({
  ...
  const getNivelItems = () => {
    ...
    return nivel
      ? [
          {
            accordionTitle: nivel.targetAudience.accordionTitle,
            content: nivel.targetAudience.targetAudience,
          },
          {
            accordionTitle: nivel.description.accordionTitle,
            content: nivel.description.description,
          },
          {
            accordionTitle: nivel.content.accordionTitle,
            content: nivel.content.content,
          },
        ]
      : [];
  };
      ...
      ? [
          {
            ...
            teacherCurriculum: professor.teacherCurriculum,
          },
        ]
      : [];
  };
  ...
  const getNivelOldValue = () => {
    const nivel = data.courseLevels.find((n) => n.id === activeNivel);
    return nivel && nivel.oldValue && nivel.oldValue !== "0"
      ? nivel.oldValue
      : null;
  };
  ...
  return (
    ...
    >
      <div
        ...
        className="relative min-w-full min-h-[50vh] overflow-hidden text-white shadow-lg rounded-xl bg-blue-gray-500 bg-clip-border shadow-blue-gray-500/40 group-data-[dropdown-value='1']:blur-sm group-data-[dropdown-value='2']:blur-sm"
      >
      ...
      {/* NÍVEIS */}
      <div
        className={`bg-slate-300 overflow-hidden text-black px-4 pt-4 h-[50vh] w-[98%] justify-self-center rounded-t-xl max-h-[50vh] absolute bottom-20 transition-all ease-linear duration-300 ${
          dropdownValue === 0 ? "scale-out" : ""
        } ${dropdownValue === 1 ? "scale-in" : "scale-out hidden"}`}
      >
        <div className="flex items-center">
          {data.courseLevels.length > 1 ? (
            <PTabBarComp
              tabs={data.courseLevels}
              className="my-auto"
              onTabClick={handleNivelTabClick}
            />
          ) : data.courseLevels.length > 0 ? (
            <div className="my-3 ml-4 text-xs sm:text-sm lg:text-lg xl:text-xl text-primary_blue font-semibold">
              {data.courseLevels[0].tabTitle === "Dados do Curso"
                ? ""
                : "Curso de Nível"}{" "}
              {data.courseLevels[0].tabTitle}
            </div>
          ) : (
            <div className="my-3 ml-4 text-xs sm:text-sm lg:text-lg xl:text-xl text-primary_blue font-semibold">
              Não Existem Dados do Curso
            </div>
          )}
          <button
      ...
      <div
        className={`bg-slate-300 overflow-hidden text-black px-4 pt-4 h-[50vh] w-[98%] justify-self-center rounded-t-xl max-h-[50vh] absolute bottom-20 transition-all ease-linear duration-300 ${
          dropdownValue === 0 ? "scale-out" : ""
        } ${dropdownValue === 2 ? "scale-in" : "scale-out hidden"}`}
      >
        <div className="flex items-center">
          {data.courseTeachers.length > 1 ? (
            <PTabBarComp
              tabs={data.courseTeachers}
              className="my-auto"
              onTabClick={handleProfessorTabClick}
            />
