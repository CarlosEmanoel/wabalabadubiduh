...
const TabBarCardComp = ({ tabs, buttonTitle, size, onClick }) => {
    ...
    const [activeSubTab, setActiveSubTab] = useState(activeTab.subTabs ? activeTab.subTabs[0] : null);
            ...
            <div className={tabBarCardVariants({ mms: finalSize })}>
                <div>
                    <div className='flex flex-col gap-3 justify-between'>
                        <PTabBarComp tabBlink={tabBlink} tabs={tabs} onTabClick={handleTabClick} />
                        {activeTab.subTabs && (
                            <PTabBarComp tabBlink={tabBlink} tabs={activeTab.subTabs} onTabClick={handleSubTabClick} />
                        )}
                    </div>
                    <div className="px-6">
                ...
                {activeTab.href &&
                    <button className="inline-block mt-4 group w-auto" onClick={onClick}>
                        <div
                            className="flex items-center gap-2 rounded-lg py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-secondary_blue transition-all hover:bg-secondary_blue/10 active:bg-secondary_blue/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        >
                            {buttonTitle}
                            <svg
